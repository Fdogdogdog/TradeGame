...
def update():
    ...
    frm_trades.configure(bg='#F0F0F0')
    ...
    if tutorial_current_page==2:
        frm_trades.configure(bg='#FFFF00')
...
frm_buildings=Frame(window, padding=10)
frm_resources=Frame(window, padding=10)
frm_trades=tk.Frame(window, padx=10, pady=10)
frm_next_turn=Frame(window, padding=10)
frm_save_load=Frame(tlv_menu, padding=10)
...
btn_build_sawmill=Button(frm_buildings, text='Build Sawmill (Cost: 50 wood, 25 stone)', command=build_sawmill)
btn_build_stone_mine=Button(frm_buildings, text='Build Stone Mine (Cost: 50 stone, 25 wood)', command=build_stone_mine)
btn_build_iron_mine=Button(frm_buildings, text='Build Iron Mine (Cost: 100 stone, 50 iron)', command=build_iron_mine)
btn_build_gold_mine=Button(frm_buildings, text='Build Gold Mine (Cost: 100 iron, 50 stone)', command=build_gold_mine)
btn_build_storehouse=Button(frm_buildings, text='Build Storehouse (Cost: 100 wood, 25 iron)', command=build_storehouse)
...
btn_trade_1=Button(frm_trades, command=do_trade_1)
btn_trade_2=Button(frm_trades, command=do_trade_2)
btn_trade_3=Button(frm_trades, command=do_trade_3)

btn_next_turn=Button(frm_next_turn, text='Next Turn', command=next_turn, style='Title.TButton')
lbl_turns_taken=Label(frm_next_turn)

btn_open_menu=Button(window, text='Menu', command=show_menu, style='Title.TButton')

# menu
btn_open_tutorial=Button(tlv_menu, text='Tutorial', command=show_tutorial, style='Title.TButton')

btn_open_whats_new=Button(tlv_menu, text='What\'s New?', command=show_whats_new, style='Title.TButton')

lbl_save_code=tk.Label(frm_save_load, text='Save Code:', font=fnt_title)
ent_save_code=Entry(frm_save_load)
btn_save_code_write=Button(frm_save_load, text='Save', command=save_code_write)
btn_save_code_read=Button(frm_save_load, text='Load', command=save_code_read)

btn_close_menu=Button(tlv_menu, text='Close Menu', command=hide_menu, style='Title.TButton')
btn_quit_game=Button(tlv_menu, text='Quit Game', command=quit_game, style='Title.TButton')

# tutorial
msg_tutorial=tk.Message(tlv_tutorial, font=fnt_main)

btn_prev_tutorial=Button(tlv_tutorial, text='Previous', command=prev_tutorial_page)
btn_next_tutorial=Button(tlv_tutorial, text='Next', command=next_tutorial_page)

btn_close_tutorial=Button(tlv_tutorial, text='Close Tutorial', command=hide_tutorial, style='Title.TButton')

# whats new
msg_whats_new=tk.Message(tlv_whats_new, text='TradeGame v1.1\n\nAdded:\nStorehouses (buildings[\'storehouse\'])\nTreasure (resources[\'treasure\'])\nImport/export blacklist (import_blacklist)(export_blacklist)\nMenu (tlv_menu)\nTutorial (tlv_tutorial)\nWhat\'s New? (tlv_whats_new)\n\nChanged:\nMoved save/load to menu\nReworked trade system & resource values (create_trade_1(), create_trade_2(), create_trade_3())(resource_values)\nUpgraded theme to ttk (tkinter.ttk)\n\nRemoved:\nNothing\n\nFixed:\nNothing', font=fnt_main)

btn_close_whats_new=Button(tlv_whats_new, text='Close What\'s New', command=hide_whats_new, style='Title.TButton')
